...
LABHOST_FILE = "lab_host_keywords.tsv"
ISO_FILE = "isolation_source_keywords.tsv"


def read_col_from_tsv(filename, col):
    with open(filename, 'r', encoding='utf-8-sig') as file:
        return list(r[col] for r in csv.DictReader(file, delimiter="\t"))
    

MANUAL_LIST = ["mice", "mouse", "Mus musculus"]
LABHOST_LIST = read_col_from_tsv(LABHOST_FILE, col="Keywords")
ISO_LIST = read_col_from_tsv(ISO_FILE, col="Keywords")


def first_matching_keyword(query_string, keywords): 
    return next((k for k in keywords if k.lower() in query_string.lower()), None)
...
def build_row(query_string):
    ...
    for keywords, flag in ((LABHOST_LIST, LAB_HOST_FLAG), (ISO_LIST, ISO_FLAG)):
        if word := first_matching_keyword(query_string, keywords):
            flags.append(flag)
    ...
    if manual_keyword:
        flags = [MANUAL_FLAG]
        row[f"{MANUAL_FLAG}_match"] = manual_keyword
...
def main():
    args = get_args()
    input_items = read_col_from_tsv(args.input, col=args.col)
